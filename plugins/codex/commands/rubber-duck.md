...
- For a long or multiline articulation, especially one containing quotes or code, pipe it to the companion over stdin with a heredoc instead of packing it into the `$ARGUMENTS` argv string, which is fragile with quoting. The companion reads piped stdin when no positional prompt is given, so keep only the `--model`/`--effort` flags as arguments and pass the articulation on stdin. Do not use `--prompt-file` here: this command's allowlist is `Bash(node:*)` only, with no `Write` or shell file-creation command, so it cannot create the temp file. Example:
  ```bash
