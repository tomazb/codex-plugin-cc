...
- For a long or multiline articulation, especially one containing quotes or code, pipe it to the companion over stdin with a heredoc instead of packing it into the `$ARGUMENTS` argv string, which is fragile with quoting. The companion's `readTaskPrompt` falls back to reading piped stdin when there is no `--prompt-file` and no positional prompt text, so pass only the `--model`/`--effort` flags as arguments (no positional articulation) and redirect the articulation in on stdin. Do not use `--prompt-file` here: this command's allowlist is `Bash(node:*)` only, with no `Write` or shell file-creation command, so it cannot create the temp file. Example:
  ```bash
  node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" rubber-duck <<'EOF'
  ...articulation...
  EOF
  ```
