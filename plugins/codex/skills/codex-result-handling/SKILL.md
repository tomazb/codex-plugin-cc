...
- For `codex:codex-rubber-duck`, present the critique using its own structure: lead with the `assessment` (`no-issues` or `issues-found`) and summary, then the findings grouped by severity bucket — `blocking`, then `non-blocking`, then `suggestion`.
- For `codex:codex-rubber-duck`, a critique is advice, not an instruction. Do not auto-apply, patch, or start implementing fixes from the critique. Report it and let the main session decide what to do.
- For `codex:codex-rubber-duck`, if the rubber duck could not run or returned nothing, say so loudly and never present it as a clean "no issues" result. An empty or failed critique is a failure, not a pass.
